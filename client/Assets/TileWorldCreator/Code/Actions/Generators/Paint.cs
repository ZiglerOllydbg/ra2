...
#pragma warning restore 0414
		...
		public override void ModifyMap(int _x, int _y, bool _value, TileWorldCreator _twc)
		{
			...
			if (_x >= 0 && _x < _twc.twcAsset.mapWidth && _y >= 0 && _y < _twc.twcAsset.mapHeight)
			{
		...
		public override void FillMap(bool _value, TileWorldCreator _twc)
		{
			...
			for (int x = 0; x < _twc.twcAsset.mapWidth; x++)
			{
				for (int y = 0; y < _twc.twcAsset.mapHeight; y++)
				{
					paintMap[x, y] = _value;
				}
			}
		...
		TileWorldCreator FindTWCComponent()
		{
			...
			if (tileWorldCreator == null)
			{
				...
				TileWorldCreator _twc = null;
#if UNITY_2022_3_OR_NEWER
				_twc = GameObject.FindFirstObjectByType<TileWorldCreator>();
#else
				_twc = GameObject.FindObjectOfType<TileWorldCreator>();
#endif
		...
		public override void DrawGizmos()
		{
			...
			for (int x = 0; x < tileWorldCreator.twcAsset.mapWidth; x++)
			{
				for (int y = 0; y < tileWorldCreator.twcAsset.mapHeight; y++)
				{

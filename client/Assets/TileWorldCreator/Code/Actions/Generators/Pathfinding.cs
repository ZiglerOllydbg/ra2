...
namespace TWC.Actions
{
	...
	public class Pathfinding : TWCBlueprintAction, ITWCAction
	{
		...
		public bool[,] Execute(bool[,] _map, TileWorldCreator _twc)
		{
			...
			var _startLayerWidth = _startLayer.GetLength(0);
			var _startLayerHeight = _startLayer.GetLength(1);

			var _targetLayerWidth = _startLayer.GetLength(0);
			var _targetLayerHeight = _startLayer.GetLength(1);
			
			
			for (int x = 0; x < _startLayerWidth; x ++)
			{
				for (int y = 0; y < _startLayerHeight; y ++)
				{
					if (_startLayer[x,y])
					{
			...
			for (int x = 0; x < _targetLayerWidth; x ++)
			{
				for (int y = 0; y < _targetLayerHeight; y ++)
				{
					if (_targetLayer[x,y])
					{

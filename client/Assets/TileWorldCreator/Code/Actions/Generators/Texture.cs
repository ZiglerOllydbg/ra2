	...
	public class Texture : TWCBlueprintAction, ITWCAction
	{
		...
		public bool[,] Execute(bool[,] map, TileWorldCreator _twc)
		{
			if (originalTexture != null)
			{
				...
				modifiedTexture = new Texture2D(_mapSizeX, _mapSizeY);
				
				// Resize texture to map size
				if (originalTexture.width != _mapSizeX || originalTexture.height != _mapSizeY)
				{
					modifiedTexture = Resize(originalTexture, _mapSizeX, _mapSizeY);
				}
				else
				{
				...
				for (int x = 0; x < _mapSizeX; x ++)
				{
					for (int y = 0; y < _mapSizeY; y ++)
					{
						try
						{
							var _pixel = _pixels[(y * _mapSizeX + x)].grayscale;

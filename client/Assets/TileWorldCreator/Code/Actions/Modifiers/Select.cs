...
namespace TWC.Actions
{
	...
	public class Select : TWCBlueprintAction, ITWCAction
	{
		...
		public bool[,] Execute(bool [,] _map, TileWorldCreator _twc)
		{
			...
			bool[,] _outputMap = new bool[mapSizeX, mapSizeY];
			
			originalMap = new bool[mapSizeX, mapSizeY];
			...
			switch (selectType)
			{
				...
				case SelectType.random:
					_outputMap = SelectRandom(_map, _twc.currentSeed);
					break;
		...
		bool[,] SelectRandom(bool[,] _map, int _rndSeed)
		{
			bool[,] _outputMap = new bool[mapSizeX, mapSizeY];
			Random.InitState(_rndSeed);

			float[] _rndNumbers = new float[mapSizeX * mapSizeY];

			for (int x = 0; x < mapSizeX; x ++)
			{
				for (int y = 0; y < mapSizeY; y ++)
				{
					_rndNumbers[x * y] = Random.Range(0f, 1f);
				}
			}


			// var _state = UnityEngine.Random.state;
			for (int x = 0; x < mapSizeX; x ++)
			{
				for (int y = 0; y < mapSizeY; y ++)
				{
		...
		bool[,] SelectByRule(bool[,] _modifyMap)
		{
			...
			bool[,] _finalMap = new bool[mapSizeX, mapSizeY];
				
			for (int r = 0; r < neighbourSelectionRules.Count; r ++)
			{
				neighbourSelectionRules[r].selectionResult = new bool[mapSizeX, mapSizeY];

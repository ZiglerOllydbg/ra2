...
namespace ZLog
{
    ...
    internal class LogZString
    {
        static Stack<LogZString>[] g_cache;//idx特定字符串长度,深拷贝核心缓存
        static Dictionary<int, Stack<LogZString>> g_secCache;//key特定字符串长度value字符串栈，深拷贝次级缓存
        static Stack<LogZString> g_shallowCache;//浅拷贝缓存
        ...
        static LogZString[] g_format_args;//存储格式化字符串值
        ...
        //带默认长度的构造
        private LogZString(int length)
        {
            _value = new string(NEW_ALLOC_CHAR, length);
        }
        //浅拷贝专用构造
        private LogZString(string value, bool shallow)
        {
            if (!shallow)
            {
                throw new NotSupportedException();
        ...
        static LogZString()
        {
            ...
            g_format_args = new LogZString[10];
        }
        //析构
        ...
        private static LogZString get(string value)
        {
            if (value == null)
                return null;
#if DBG
        ...
        //由string浅拷贝入zstring
        private static LogZString getShallow(string value)
        {
            ...
            {
                result = new LogZString(value, true);
            }
            else
            {
        ...
        //手动添加方法
        private static void getStackInCache(int index, out Stack<LogZString> outStack)
        {
            ...
            else//从次级缓存中取
            {
                ...
                {
                    outStack = new Stack<LogZString>(INITIAL_STACK_CAPACITY);
                    g_secCache[index] = outStack;
                }
            }
        }
        //获取特定长度zstring
        private static LogZString get(int length)
        {
        ...
        //获取格式化字符串
        private unsafe static LogZString internal_format(string input, int num_args)
        {
            if (input == null)
            ...
            for (int i = -3; ;)
            {
                ...
                LogZString arg = g_format_args[arg_idx];
                new_len += arg.Length;
            }
            LogZString result = get(new_len);
        ...
        private unsafe static LogZString internal_remove(string input, int start, int count)
        {
            if (start < 0 || start >= input.Length)
            ...
            LogZString result = get(input.Length - count);
        ...
        private unsafe static LogZString internal_replace(string value, string old_value, string new_value)
        {
        ...
        private unsafe static LogZString internal_insert(string value, char to_insert, int start, int count)
        {
        ...
        private unsafe static LogZString internal_insert(string input, string to_insert, int start)
        {
            if (input == null)
        ...
        //字符串拼接
        private unsafe static LogZString internal_concat(string s1, string s2)
        {
            int total_length = s1.Length + s2.Length;
            LogZString result = get(total_length);
            fixed (char* ptr_result = result._value)
            {
        ...
        {
            readonly Stack<LogZString> stack;

            internal zstring_block(int capacity)
            {
                stack = new Stack<LogZString>(capacity);
            }

            internal void push(LogZString str)
            {
                stack.Push(str);
            }
            ...
            void IDisposable.Dispose()//析构函数
            {
                ...
                LogZString.g_blocks.Push(this);//将自身push入缓存栈

                //赋值currentBlock
                g_open_blocks.Pop();
                if (g_open_blocks.Count > 0)
                {
                    LogZString.g_current_block = g_open_blocks.Peek();
                }
                else
                {
                    LogZString.g_current_block = null;
                }
            }
        }
        ...
        {
            g_cache = new Stack<LogZString>[cache_capacity];
            g_secCache = new Dictionary<int, Stack<LogZString>>(cache_capacity);
            ...
            g_shallowCache = new Stack<LogZString>(shallowCache_capacity);
            for (int c = 0; c < cache_capacity; c++)
            {
                var stack = new Stack<LogZString>(stack_capacity);
                for (int j = 0; j < stack_capacity; j++)
                    stack.Push(new LogZString(c));
                g_cache[c] = stack;
            }
            ...
            {
                g_shallowCache.Push(new LogZString(null, true));
            }
        }
        ...
        //bool->zstring转换
        public static implicit operator LogZString(bool value)
        {
            return get(value ? "True" : "False");
        }

        // long - >zstring转换
        public unsafe static implicit operator LogZString(long value)
        {
            // e.g. 125
        ...
        //int->zstring转换
        public unsafe static implicit operator LogZString(int value)
        {
            // e.g. 125
            // first pass: count the number of digits
            // then: get a zstring with length = num digits
        ...
        //float->zstring转换
        public unsafe static implicit operator LogZString(float value)
        {
            // e.g. 3.148
            bool negative = value < 0;
        ...
        //string->zstring转换
        public static implicit operator LogZString(string value)
        {
            //return get(value);
            return getShallow(value);
        }
        //string->zstring转换
        public static LogZString shallow(string value)
        {
            return getShallow(value);
        }
        //zstring->string转换
        public static implicit operator string(LogZString value)
        {
            return value._value;
        }
        //+重载
        public static LogZString operator +(LogZString left, LogZString right)
        {
            return internal_concat(left, right);
        }
        //==重载
        public static bool operator ==(LogZString left, LogZString right)
        {
        ...
        //转换为大写
        public unsafe LogZString ToUpper()
        {
        ...
        //转换为小写
        public unsafe LogZString ToLower()
        {
            var result = get(Length);
            fixed (char* ptr_this = this._value)
            {
        ...
        //移除剪切
        public LogZString Remove(int start)
        {
            return Remove(start, Length - start);
        }
        //移除剪切
        public LogZString Remove(int start, int count)
        {
            return internal_remove(this._value, start, count);
        }
        //插入start起count长度字符
        public LogZString Insert(char value, int start, int count)
        {
            return internal_insert(this._value, value, start, count);
        }
        //插入start起字符串
        public LogZString Insert(string value, int start)
        {
            return internal_insert(this._value, value, start);
        }
        //子字符替换
        public unsafe LogZString Replace(char old_value, char new_value)
        {
            LogZString result = get(Length);
            fixed (char* ptr_this = this._value)
            {
                fixed (char* ptr_result = result._value)
                {
        ...
        //子字符串替换
        public LogZString Replace(string old_value, string new_value)
        {
            return internal_replace(this._value, old_value, new_value);
        }
        //剪切start位置起后续子串
        public LogZString Substring(int start)
        {
            return Substring(start, Length - start);
        }
        //剪切start起count长度的子串
        public unsafe LogZString Substring(int start, int count)
        {
        ...
        //自身+value拼接
        public LogZString Concat(LogZString value)
        {
            return internal_concat(this, value);
        }
        //静态拼接方法簇
        public static LogZString Concat(LogZString s0, LogZString s1) { return s0 + s1; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2) { return s0 + s1 + s2; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3) { return s0 + s1 + s2 + s3; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4) { return s0 + s1 + s2 + s3 + s4; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4, LogZString s5) { return s0 + s1 + s2 + s3 + s4 + s5; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4, LogZString s5, LogZString s6) { return s0 + s1 + s2 + s3 + s4 + s5 + s6; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4, LogZString s5, LogZString s6, LogZString s7) { return s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4, LogZString s5, LogZString s6, LogZString s7, LogZString s8) { return s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8; }

        public static LogZString Concat(LogZString s0, LogZString s1, LogZString s2, LogZString s3, LogZString s4, LogZString s5, LogZString s6, LogZString s7, LogZString s8, LogZString s9) { return s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9; }
        //静态格式化方法簇
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4, LogZString arg5, LogZString arg6, LogZString arg7, LogZString arg8, LogZString arg9)
        {
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4, LogZString arg5, LogZString arg6, LogZString arg7, LogZString arg8)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4, LogZString arg5, LogZString arg6, LogZString arg7)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4, LogZString arg5, LogZString arg6)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4, LogZString arg5)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3, LogZString arg4)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2, LogZString arg3)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1, LogZString arg2)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
            if (arg2 == null) throw new ArgumentNullException("arg2");
        ...
        public static LogZString Format(string input, LogZString arg0, LogZString arg1)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
            if (arg1 == null) throw new ArgumentNullException("arg1");
        ...
        public static LogZString Format(string input, LogZString arg0)
        {
            if (arg0 == null) throw new ArgumentNullException("arg0");
        ...
        public static LogZString FloatToZstring(float value, uint DecimalAccuracy)
        {
            uint oldValue = LogZString.DecimalAccuracy;
            LogZString.DecimalAccuracy = DecimalAccuracy;
            LogZString target = (LogZString)value;
            LogZString.DecimalAccuracy = oldValue;
            return target;
        }

        //判空或长度
        public static bool IsNullOrEmpty(LogZString str)
        {
            return str == null || str.Length == 0;
        }
        //是否以value结束
        public static bool IsPrefix(LogZString str, string value)
        {
            return str.StartsWith(value);
        }
        //是否以value开始
        public static bool isPostfix(LogZString str, string postfix)
        {

...
public class ModuleDiscover : BaseDiscover
{
    ...
    public override void OnDiscoverModule(Type _type, Attribute _attribute)
    {
        if (_type.IsSubclassOf(typeof(Module)))
        {
            ...
            {
                Debug.LogError($"ModuleAttribute key is repeat! TemplateName : { moduleAttribute.moduleName } type : { _type.FullName }");
            }
        }
        else
        {
            Debug.LogError($"Not Allow a module not extend Module! { _type.FullName }");
        }
    }
